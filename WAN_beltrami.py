...
class WAN3DNS:
    ...
    def __init__(self, x0, y0, z0, t0, u0, v0, w0, xb, yb, zb, tb, ub, vb, wb, x, y, z, t, layers1, layers2):
        ...
        self.f_u_pred, self.f_v_pred, self.f_w_pred, self.f_e_pred = self.net_f_NS(self.x_tf, self.y_tf, self.z_tf, self.t_tf)
        self.s1_pred, self.s2_pred, self.s3_pred, self.l_pred, self.m_pred, self.n_pred = self.net_test(self.x_tf, self.y_tf, self.z_tf, self.t_tf)
        alpha = 10000
        beta = 10000

        # set loss function
        self.loss1 = alpha * tf.reduce_mean(tf.square(self.u_ini_tf - self.u_ini_pred)) + \
...
# ###################without assume###############
    ...
    def net_NS(self, x, y, z, t):
        ...
        u_v_w_p = self.neural_net(tf.concat([x, y, z, t], 1), self.weights1, self.biases1)
        u = u_v_w_p[:, 0:1]
        v = u_v_w_p[:, 1:2]
        w = u_v_w_p[:, 2:3]
        p = u_v_w_p[:, 3:4]
    ...
    def net_test(self, x, y, z,t):

        s_l_m_n = self.neural_test(tf.concat([x, y, z, t], 1), self.weights2, self.biases2)
        s1 = s_l_m_n[:, 0:1]
        s2 = s_l_m_n[:, 1:2]
        s3 = s_l_m_n[:, 2:3]
        l = s_l_m_n[:, 3:4]
    ...
    def net_f_NS(self, x, y, z, t):
        ...
        f_u = u_t * l + u_x * l_x / Re + u * u_x * l + u * u_y * l + u * u_z * l
        f_v = v_t * m + v_y * m_y / Re + v * v_x * m + v * v_y * m + v * v_z * m
        f_w = w_t * n + w_z * n_z / Re + w * w_x * n + w * w_y * n + w * w_z * n
...
# train的tf_dict需要修改
    ...
    def Adam_train(self, nIter=20000, learning_rate1=1e-3, learning_rate2=1e-3):
        tf_dict = {
            self.x_ini_tf: self.x0, self.y_ini_tf: self.y0, 
            self.z_ini_tf: self.z0, self.t_ini_tf: self.t0,
            self.u_ini_tf: self.u0, self.v_ini_tf: self.v0, 
            self.w_ini_tf: self.w0,
            self.x_boundary_tf: self.xb, self.y_boundary_tf: self.yb,
            self.z_boundary_tf: self.zb, self.t_boundary_tf: self.tb,
            self.u_boundary_tf: self.ub, self.v_boundary_tf: self.vb,
            self.w_boundary_tf: self.wb, 
            self.x_tf: self.x, self.y_tf: self.y, 
            self.z_tf: self.z, self.t_tf: self.t,
...
if __name__ == "__main__":
    ...
    layers1 = [4, 20, 20, 20, 20, 20, 20, 20, 20, 4]
    layers2 = [4, 30, 30, 30, 30, 30, 30, 6]
    # Load Data
    ...
    model = WAN3DNS(x0_train, y0_train, z0_train, t0_train,
                     u0_train, v0_train, w0_train,
                     xb_train, yb_train, zb_train, tb_train,
                     ub_train, vb_train, wb_train,
                     x_train, y_train, z_train, t_train, layers1, layers2)

    model.Adam_train(nIter=5000, learning_rate1=1e-3, learning_rate2=1e-3)
    # model.BFGS_train()
