...
class WeakFormNS:
    ...
    def create_test_functions(self, x, num_test_funcs=10):
        ...
        for i in range(num_test_funcs):
            ...
            # 创建满足散度为零的测试函数
            # 在二维中，我们可以使用旋度形式：v = (∂ψ/∂y, -∂ψ/∂x)
            # 这里我们使用 w_val 作为流函数 ψ
    ...
    def calculate_l2_errors(self, sess, x_test):
        ...
        u_pred = u_v_p_pred[:, 0]  # 第一个分量是u
        v_pred = u_v_p_pred[:, 1]  # 第二个分量是v
        p_pred = u_v_p_pred[:, 2]  # 第三个分量是p
...
if __name__ == '__main__':
    ...
    with tf.compat.v1.Session() as sess:
        ...
        print(f"Best relative error - u: - v: {l2_v}")
        print(f"Best relative error - u: - p: {l2_p}")
        ...
        print("Results has been saved: WAN3DNS_kov_results.npy")